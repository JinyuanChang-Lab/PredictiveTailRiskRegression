...
class low_dim():
    ...
    def __init__(self, X, Y, intercept=True, options=dict()):
        ...
        if intercept:
            is_all_ones = (X[:, 0] == 1).all()
            if is_all_ones:
                self.mX, self.sdX = np.mean(X[:,1:], axis=0), np.std(X[:,1:], axis=0)
                self.X = X[:]
                self.X1 = np.c_[np.ones(self.n), (X[:,1:] - self.mX)/self.sdX]
            else:
...
class joint(low_dim):
    ...
    def fz(self, tau=0.5, G1=False, G2_type=1,
        ...
        qrfit = low_dim(self.X[:, self.itcp:], self.Y, intercept=self.itcp)\
    ...
    def twostep(self, tau=0.5, h=None, kernel='Laplacian', 
                loss='L2', robust=None, new = False, G2_type=1,
        ...
        if loss == 'L2':
            ...
            if robust == None:
                if new == True:
                    robust = 0.25 * np.mean((np.minimum(qrfit['res'], 0) / tau) ** 4) ** 0.25 \
                            * (self.n / self.X.shape[1] / (np.log(self.n) ** 2)) ** (1 / 6)
