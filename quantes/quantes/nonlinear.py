...
class LocPoly:
    ...
    def ls(self, x0, bw=.5, degree=1):
        ...
        class sol: x = np.linalg.pinv(ZW @ Z, rcond = 1e-6) @ (ZW @ self.Y)
        self.bias_m = sol.x[0]
        self.coef_m = sol.x[1:]
        self.sol_m = sol
...
class FNN:
    ...
    def __init__(self, X, Y, normalization=None):
        ...
        self.const_cols = np.std(X, axis=0) == 0
        self.nonconst_cols = ~self.const_cols
        ...
        #     self.xmin = np.min(X[:, self.nonconst_cols], axis=0)
        #     self.xmax = np.max(X[:, self.nonconst_cols], axis=0)
    
        #     self.X0[:, self.nonconst_cols] = (
        #         X[:, self.nonconst_cols] - self.xmin
        #     ) / (self.xmax - self.xmin)
    
        #     # 常数列，比如截距列，保持不变
        #     self.X0[:, self.const_cols] = X[:, self.const_cols]
        # elif self.nm == 'Z-score':
        #     self.X0 = X.copy()
    
        #     self.xm = np.mean(X[:, self.nonconst_cols], axis=0)
        #     self.xsd = np.std(X[:, self.nonconst_cols], axis=0)
    
        #     self.X0[:, self.nonconst_cols] = (
        #         X[:, self.nonconst_cols] - self.xm
        #     ) / self.xsd
        ...
        # else:
        #     raise ValueError("normalization should be one of [None, 'MinMax', 'Z-score']")
    ...
    #     elif self.nm == 'MinMax':
    #         X0 = X.copy()
    #         X0[:, self.nonconst_cols] = (
    #             X[:, self.nonconst_cols] - self.xmin
    #         ) / (self.xmax - self.xmin)
    #         # 截距列/常数列保持不变
    #         X0[:, self.const_cols] = X[:, self.const_cols]
    #     elif self.nm == 'Z-score':
    #         X0 = X.copy()
    #         X0[:, self.nonconst_cols] = (
    #             X[:, self.nonconst_cols] - self.xm
    ...
    #     else:
    #         raise ValueError("normalization should be one of [None, 'MinMax', 'Z-score']")
    #     Xnew = torch.as_tensor(X0, dtype=torch.float).to(self.device)
    #     return self.model.predict(Xnew)


    def trainer(self, x, y, loss_fn, output_dim=1,
                device='cpu', val_fn=None, name='', save_best_model=False):
        '''
